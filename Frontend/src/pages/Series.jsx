...
const Series = () => {
  ...
  const fetchSeries = async () => {
    try {
      setLoading(true)
      let url = `${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&page=${currentPage}&sort_by=${sortBy}`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12 relative">
        ...
        <div className="mb-12 text-center animate-fade-in-up">
          <div className="flex items-center justify-center space-x-4 mb-6">
            ...
            <FontAwesomeIcon icon={faTv} className="text-5xl text-cyan-400 animate-glow-pulse" />
            <div className="w-16 h-1 bg-gradient-to-r from-transparent via-purple-500 to-transparent"></div>
          </div>
          
          <h1 className="text-6xl md:text-7xl font-serif font-black mb-4 text-white">
            Discover Series
          </h1>
            ...
            <div className="flex items-center gap-4">
              <div className="relative group">
                ...
                >
                  ...
                  <option value="first_air_date.desc" className="bg-cinematic-card text-white">Newest First</option>
                  <option value="vote_average.desc" className="bg-cinematic-card text-white">Highest Rated</option>
                  <option value="name.asc" className="bg-cinematic-card text-white">A-Z</option>
                </select>
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="glass-dark px-6 py-3 rounded-xl border border-white/10 inline-block">
            <p className="text-gray-300 font-medium">
              <span className="text-cyan-400 font-bold">{series.length}</span> of{' '}
              <span className="text-purple-400 font-bold">{totalResults.toLocaleString()}</span> series
        ...
        {loading ? (
          ...
          <div className={viewMode === 'grid' 
            ...
            {series.map((show, index) => (
              viewMode === 'grid' ? (
                <div key={show.id} className={`animate-fade-in-up stagger-${(index % 8) + 1}`}>
                  <MovieCard
                    movie={{
                      imdbId: `tmdb${show.id}`,
                      title: show.name,
                      year: show.first_air_date?.split('-')[0],
                      poster: show.poster_path ? `${TMDB_IMAGE_BASE}${show.poster_path}` : '/placeholder-movie.svg',
                      imdbRating: show.vote_average?.toFixed(1),
                      runtime: 'N/A',
                      genre: show.genre_ids?.join(', ') || 'N/A',
                      plot: show.overview,
                      type: 'series'
                    }}
                    onAddToWatchlist={async () => {
                      try {
                        await watchlistAPI.add(`tmdb${show.id}`)
                    ...
                    onAddToFavorites={async () => {
                      try {
                        await favoritesAPI.add(`tmdb${show.id}`)
                ...
                <div key={show.id} className="glass-card rounded-2xl p-6 flex gap-6 hover:shadow-[0_0_30px_rgba(0,212,255,0.2)] transition-all duration-500 animate-fade-in-up border border-white/5 hover:border-cyan-400/30">
                  <img
                    src={show.poster_path ? `${TMDB_IMAGE_BASE}${show.poster_path}` : '/placeholder-movie.svg'}
                    alt={show.name}
                    className="w-24 h-36 object-cover rounded-xl shadow-lg"
                  />
                  <div className="flex-1">
                    <h3 className="font-serif font-bold text-xl mb-3 text-white hover:text-electric-blue transition-colors">
                      {show.name}
                    </h3>
                    <div className="flex items-center gap-4 text-sm text-gray-300 mb-3">
                      <span className="flex items-center gap-2 glass-dark px-3 py-1.5 rounded-lg">
                        ...
                        <span className="font-medium">{show.first_air_date?.split('-')[0]}</span>
                      </span>
                      <span className="flex items-center gap-2 glass-dark px-3 py-1.5 rounded-lg">
                    ...
                    <p className="text-gray-400 text-sm line-clamp-2 leading-relaxed">{show.overview}</p>
                  </div>
                </div>
              )
            ))}
        ...
        {!loading && series.length === 0 && (
          <div className="text-center py-20">
            <div className="glass-glow rounded-3xl p-12 max-w-2xl mx-auto border-neon-purple">
              <div className="mb-6">
                <FontAwesomeIcon icon={faTv} className="text-7xl text-purple-400 opacity-50" />
              </div>
              <h3 className="text-3xl font-serif font-bold mb-4 text-white">No Series Found</h3>
              <p className="text-xl text-gray-400 mb-8">We couldn't find any series matching your criteria</p>
              <button
                onClick={() => {
                  setFilters({ genre: '', year: '' })
                  setCurrentPage(1)
                }}
                className="px-8 py-4 bg-electric-blue hover:bg-[#0080dd] rounded-xl font-bold text-white transition-all duration-300 hover:scale-105"
              >
                Clear All Filters
              </button>
            </div>
